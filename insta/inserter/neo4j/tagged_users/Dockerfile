...
RUN go mod download
COPY . .
RUN GOOS=linux go build --tags seabolt_static -o neo4j_user-inserter insta/inserter/neo4j/tagged_users/main.go

FROM alpine
RUN apk --no-cache add ca-certificates
RUN mkdir /app
COPY http_header-generator/useragents.json /app
COPY --from=builder /app/neo4j_user-inserter /app
WORKDIR /app
CMD ["./neo4j_user-inserter"]
